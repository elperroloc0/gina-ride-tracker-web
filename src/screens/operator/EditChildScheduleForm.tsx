...
import { ApiError, createChildSchedule, deleteChildSchedule, updateChildSchedule } from '../../api/client';
 ...
 * shape rather than allowing a different time per day.
 */
export function EditChildScheduleForm({ child, onDone, onCancel }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
      <div style={{ fontFamily: 'var(--display)', fontSize: 20, textTransform: 'uppercase' }}>Edit {child.name}&rsquo;s schedule</div>
